...
describe('Page results (Nuxt runtime)', () => {
  ...
  it('quand q est présent, affiche "Résultats pour" et appelle searchOffers', async () => {
    const wrapper = await mountSuspended(ResultsPage, { route: '/results?q=test' })
    await vi.waitFor(
      () => {
        expect(wrapper.text()).toContain('Résultats pour')
        expect(wrapper.text()).toContain('"test"')
        expect(mockSearchOffers).toHaveBeenCalledWith('test')
      },
      { timeout: 2000 },
    )
  })
