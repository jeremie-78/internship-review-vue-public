...
describe('Nuxt structure (AC #1, #7)', () => {
  it('nuxt.config has alias and Tailwind via Vite', async () => {
  ...
  it('DefaultLayout and home-page exist (AC #3, #4)', () => {
    expect(existsSync(join(appDir, 'layouts', 'DefaultLayout.vue'))).toBe(true)
    expect(existsSync(join(appDir, 'pages', 'home-page.vue'))).toBe(true)
    const layoutContent = readFileSync(join(appDir, 'layouts', 'DefaultLayout.vue'), 'utf-8')
